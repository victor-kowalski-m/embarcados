...
boolean ESP8266::conectaRede(){//returns a '1' if successful
  ...
  if(this->buscaTrecho(keyword_OK,sizeof(keyword_OK),5000,0))//go look for keyword "OK" with a 5sec timeout
    //##Serial.println("ESP CHECK OK");
    ;
  else
    //##Serial.println("ESP CHECK FAILED");
    ;
  this->limpaSerial();//this just reads everything in the buffer and what's still coming from the ESP

  Serial.print("AT+RST\r\n");// Give it a reset - who knows what condition it was in, better to start fresh
  if(this->buscaTrecho(keyword_Ready,sizeof(keyword_Ready),5000,0))//go look for keyword "Ready" - takes a few seconds longer to complete
  ...
  Serial.print("AT+CWMODE=");// set the CWMODE
  Serial.print(CWMODE);//just send what is set in the constant
  Serial.print("\r\n");
  if(this->buscaTrecho(keyword_OK,sizeof(keyword_OK),1000,0))//go look for keyword "OK"
  ...
  this->limpaSerial();  
   ...
   Serial.print("AT+CWJAP=\"");// set the SSID AT+CWJAP="SSID","PW"
   Serial.print(SSID);//from constant 
   Serial.print("\",\"");
   Serial.print(SENHA);//form constant
   Serial.print("\"\r\n");
  if(this->buscaTrecho(keyword_OK,sizeof(keyword_OK),10000,0))//go look for keyword "OK"
  ...
  if(this->buscaTrecho(keyword_rn,sizeof(keyword_rn),10000,0)){//look for first \r\n after AT+CIFSR echo - note mode is '0', the ip address is right after this
    if(this->buscaTrecho(keyword_rn,sizeof(keyword_rn),1000,1)){//look for second \r\n, and store everything it receives, mode='1'
  ...
  Serial.print("AT+CIPMUX=");// set the CIPMUX
  Serial.print(CIPMUX);//from constant
  Serial.print("\r\n");
  if(this->buscaTrecho(keyword_OK,sizeof(keyword_OK),5000,0)){//go look for keyword "OK" or "no change 
...
boolean ESP8266::buscaTrecho(const char keyword1[], int key_size, int timeout_val, byte mode){
  ...
  for(byte i=0; i<key_size; i++){//we only need a buffer as long as the keyword
    ...
    data_in[i]=Serial.read();// save the byte to the buffer 'data_in[]
...
//THE BUFFER IS FULL, SO START ROLLING NEW DATA IN AND OLD DATA OUT
  while(1){//stay in here until the keyword found or a timeout occurs
   ...
   //timing control
    while(!Serial.available()){// same thing as done in the buffer
    ...
    data_in[key_size-1]=Serial.read();//save the new data in the last position in the buffer
...
boolean ESP8266::conectaServer(){//returns 1 if successful or 0 if not

  //##Serial.println("CONNECTING");
  Serial.print("AT+CIPSTART=0,\"TCP\",\"");
  Serial.print(IP_SERVER);
  Serial.print("\",8080\r\n");//connect to your web server
  //this->buscaTrecho(keyword,size of the keyword,timeout in ms, data save 0-no 1-yes 'more on this later') 
  if(this->buscaTrecho(keyword_OK,sizeof(keyword_OK),5000,0)){//go look for 'OK' and come back
  this->limpaSerial();//get rid of whatever else is coming
  //##Serial.println("CONNECTED");//yay, connected
  Serial.print("AT+CIPSEND=0,");//send AT+CIPSEND=0, size of payload
  Serial.print(payload_size);//the payload size
  this->limpaSerial();//everything is echoed back, so get rid of it
  Serial.print("\r\n");//cap off that command with a carriage return and new line feed
  
  if(this->buscaTrecho(keyword_carrot,sizeof(keyword_carrot),5000,0)){//go wait for the '>' character, ESP ready for the payload
    //##Serial.println("READY TO SEND");
    for(int i=0; i<payload_size; i++)//print the payload to the ESP
    Serial.print(payload[i]);
    
    if(this->buscaTrecho(keyword_sendok,sizeof(keyword_sendok),5000,0)){//go wait for 'SEND OK'
  ...
  this->conectaRede();//optional, this will go conectaRede the module and attempt to repair itself - connect to SSID, set the CIPMUX, etc...
  }
...
void ESP8266::fazRequest(char codigoDeBarras[]){
  ...
  if(this->conectaServer()){//this calls 'this->conectaServer()' and expects a '1' back if successful
  //nice, we're in and ready to look for data
  //first up, we need to parse the returned data  _t1123##_d15##_d210##

    if(this->buscaTrecho(keyword_doublehash,sizeof(keyword_doublehash),5000,0)){//go find t1 then stop
      if(this->buscaTrecho(keyword_doublehash,sizeof(keyword_doublehash),5000,1)){//our data is next, so change mode to '1' and stop at ##
        ...
        resposta_site[0] = (scratch_data_from_ESP[0]-sizeof(keyword_doublehash)+1);
            ...
            if(this->buscaTrecho(keyword_linkdisc,sizeof(keyword_linkdisc),5000,0)){
              ...
              this->limpaSerial();//now we can clear out the buffer and read whatever is still there
